...
`onSignedIn()`: calls `fullSync()`, then (regardless of whether it's the very first sign-in this session or a page reload where a session already existed) sets `lastPullAt = Date.now()` and starts: a 60-second `setInterval` calling `pullSince(lastPullAt)` then updating `lastPullAt`; a `focus` listener doing the same on demand; an `online` listener calling `fullSync()` (a reconnect deserves the thorough reconciliation, matching `04-BACKEND-SUPABASE-SYNC.md`'s original "on reconnect, full pull" intent). Each of these three also sets `status: 'synced', lastSyncedAt: Date.now()` after the call resolves (they always resolve, never reject, per `lib/sync.ts`'s existing contract) — **and also calls `useUserDataStore.getState().loadUserData()`**, re-reading all five IndexedDB stores into the in-memory Zustand state. **(Correction, found during the whole-branch review: an earlier version of this sub-project omitted this. `fullSync`/`pullSince` write pulled rows straight to IndexedDB — correct — but nothing ever refreshed `userDataStore`'s in-memory copy afterward, so a background pull silently updated storage while every screen kept rendering stale data until a manual page reload, defeating the entire point of auto-sync.)** `syncStore.ts` statically imports `userDataStore.ts` for this — safe for bundling, since `userDataStore.ts` is already in the eager main chunk (via `App.tsx`) regardless, and `syncStore.ts` is already its own separate lazy chunk (reached only via dynamic imports) regardless of this added edge.
...
**`scheduleDirtyPush()`**: a shared debounce (3000ms constant, named `DIRTY_PUSH_DEBOUNCE_MS`) — each call clears any pending timeout and sets a new one calling `pushDirty()` (then the same `markSynced()` finish — status/timestamp update plus the `loadUserData()` refresh described above). The refresh is a no-visible-op in the common case (a pure push doesn't change what's in IndexedDB — `userDataStore`'s in-memory copy was already updated synchronously by the local write that triggered this push, so re-reading it back just confirms the same values), but sharing one `markSynced()` finish for every sync path is simpler than special-casing "this path pulls, that one doesn't," and re-reading ~160 rows across 5 stores at most once per 3-second debounce window is not a real cost at this app's scale. Calling this while signed out is harmless: `pushDirty()` already no-ops instantly (checks `getSupabase()`/session itself), so `scheduleDirtyPush()` never needs to know or check auth state. A `visibilitychange` listener (added once, in `start()`) flushes any pending debounce immediately when the tab becomes hidden — same "don't lose an edit on the way out" reasoning as `TopicNotes.tsx`'s existing flush-on-blur.

**`syncNow()`**: the manual button's handler. Sets `status` unchanged (see Section 3 on why there's no `'syncing'` state), calls `fullSync()` directly (full reconciliation — appropriate for an infrequent, user-initiated action), then the same `markSynced()` finish as every other path (timestamp update plus `loadUserData()` refresh).

## Section 2: `userDataStore.ts` — four call sites

Each of `setStatus`, `toggleFavorite`, `setNote`, `gradeCard` gets one added line, immediately after its existing `void persistX(...)` call, calling into `syncStore`'s debounced push. **Not a static top-level `import { useSyncStore } from './syncStore'`** — `userDataStore.ts` is itself eagerly bundled (statically imported by `App.tsx`), so a static import here would pull `syncStore.ts` and everything it reaches (`authStore.ts`, `lib/sync.ts`, `lib/supabase.ts`, `@supabase/supabase-js`) into the eager main chunk with it, defeating `App.tsx`'s own dynamic imports of those same modules. **(Correction, found during implementation:** this is exactly what an early version of this sub-project shipped — a static import here grew the eager bundle from ~521KB to ~742KB before it was caught and fixed. The actual call site is a small local helper using a per-call dynamic import instead: `function scheduleSyncPush(): void { void import('./syncStore').then(({ useSyncStore }) => useSyncStore.getState().scheduleDirtyPush()); }`, with each of the four actions calling `scheduleSyncPush()`.) `recordView` is untouched — `recents` never syncs (established in #3a).
...
- `kb-app/src/store/syncStore.ts` exports `useSyncStore` with the shape in Section 1, and every path through `markSynced()` refreshes `userDataStore` via `loadUserData()`.
- `kb-app/src/store/userDataStore.ts`'s four syncable actions trigger `syncStore`'s debounced push via a per-call dynamic import (never a static top-level import of `syncStore.ts` — see Section 2); `recordView` does not.
