...
The original doc's "<500KB first-load" conflates two different measurements: Vite's build-time chunk-size warning (500KB **minified**, currently tripped by the single 719.60KB bundle) and actual network transfer size (**gzip**, already 205.34KB today, comfortably under any reasonable budget — gzip is what a browser actually downloads). Splitting Map's chunk out is expected to bring the remaining eager bundle under Vite's 500KB-minified warning threshold on its own. **Concrete, checkable Definition-of-Done criterion: `npm run build` produces no "chunks are larger than 500 kB" warning** — not a specific KB target, which the original doc never precisely defined.
